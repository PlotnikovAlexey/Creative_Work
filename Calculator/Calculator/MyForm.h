...
namespace Calculator {
	...
	//Function to calculate the arc cosine
	...
	//Function for calculating the arcsine
	double asinus(double x) {
		return System::Math::Asin(x);
	}
	//Function for calculating tangents
	double tangens(double x) {
		return System::Math::Tan(x);
	}
	//Function for calculating arc tangents
	...
	//Translation function to type Int
	int toint(System::String^ str, int y) {
		switch (str[y])
		{
		case '0': return 0;
		case '1': return 1;
	...
	//Function to check if the next character is a number
	...
	//Function to check if the previous character is a number
	...
	private: System::Void button1_Click(System::Object^ sender, System::EventArgs^ e) {
		if (Protect()) {
			...
			if (comboBox1->SelectedItem == "Алгебраическое уравнение") {
					...
					if (s[i] <= '9' && s[i] >= '0') {
						...
						if (!nextch(s, i)) {
							if (prevch(s, i) && prevch(s, i - 1)) {
								if (s[i - 3] == '-') {
									nums[i] = -(toint(s, i - 2) * 100 + toint(s, i - 1) * 10 + toint(s, i));
								}
								else {
									nums[i] = toint(s, i - 2) * 100 + toint(s, i - 1) * 10 + toint(s, i);
								}
							...
							if (!prevch(s, i - 1) && prevch(s, i)) {
								if (s[i - 2] == '-') {
									nums[i] = -(toint(s, i - 1) * 10 + toint(s, i));
								}
								else {
									nums[i] = toint(s, i - 1) * 10 + toint(s, i);
								}
				...
				// Calculation of the result by the method of half division
				Result = 0;
				while ((b - a) > e) {
					c = (a + b) / 2;
			...
			if (comboBox1->SelectedItem == "Трансцендентное уравнение") {
					...
					// Calculating odds without x
					prom = 0;
					for (int i = 0; i < 100; i++) {
						prom = prom + nums[i] + mn[i] + si[i] + acosin[i] + cosin[i] + asi[i] + tan[i] + atan[i] + logs[i];
					}
					// Calculating the result if x is under the sign of the logarithm
					if (logf) {
						Result = 0;
						while ((b - a) > e) {
							c = (a + b) / 2;
					...
					// Calculating the result if x is not under the sign of the logarithm
					if (!logf) {
						Result = 0;
						while ((b - a) > e) {
							c = (a + b) / 2;
...
private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	textBox4->Text = "";
		...
		// Invalid input protection function
		   bool Protect() {
